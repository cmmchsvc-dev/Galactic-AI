...
def patch_gateway_v2():
    ...
    while i < len(lines):
        ...
        if 'if publisher == "google":' in line and i + 1 < len(lines) and 'url = f"https://{loc}-aiplatform' in lines[i+1]:
            # Found the location. We want to insert our special partner routing BEFORE or replacing the construction
            new_lines.append(line)
            new_lines.append(lines[i+1])
            i += 2
            
            # Now handle the else (partner models)
            if 'else:' in lines[i]:
                new_lines.append(lines[i])
                i += 1
                # Skip the old partner logic
                while i < len(lines) and 'try:' not in lines[i]:
                    i += 1
                
                # Insert our new partner URL logic
                new_lines.append('                        # Partner Models (MiniMax/GLM) route to OpenAI-compatible endpoint in global\n')
                new_lines.append('                        if loc == "global" and publisher in ("minimax", "minimax-ai", "minimaxai"):\n')
                new_lines.append('                            # Specific mapping to the billing ID provided by the user\n')
                new_lines.append('                            mid_effective = "minimaxai/minimax-m2-maas" if "minimax-m2" in mid.lower() else mid\n')
    ...
    print("Patch V2 applied to gateway_v3.py")
