## v2.2.0 — Working Hybrid Coder, Project Workspaces & a Pro CLI (2026-07-22)

Headline features: **Project Workspaces + searchable conversation history** (Antigravity-style), a genuinely **working hybrid coding loop** (cloud Architect on Moonshot/Kimi K3 plans, local Ollama Builder executes — proven end to end after a long bug hunt), **LM Studio** as a first-class local backend, real **context-window auto-detection** (Kimi K3 = 1M), and a **professional CLI** (framed input, live status bar, `/model` + `/workspace` switchers, inline history, approval gates). The detailed entries below (originally logged under 2.1.8) make up this release.



- 🚫 **Hallucination guard no longer gaslights the model**: the gateway's browser/file "phantom action" detectors matched bare substrings anywhere in the raw response — so Qwen *explaining* that reasoning models pause "before they start typing" got hit with "⚠️ HALLUCINATION DETECTED (clicking/typing/searching)" mid-conversation with no browser task in sight (the model then spent a think-block defending itself, correctly). "re**searching**", "proto**typing**", "c**entered**", "found the issue", and "I've created a table below" were all tripwires too. The detectors now (1) strip `<think>` blocks first — reasoning *about* an action is not a claim of action; (2) require a real first-person or narrated claim on word boundaries ("I clicked…", "Now clicking the…"); (3) arm the browser guard only after a `chrome_*`/`browser_*` tool has actually been used in the task, so pure chat can never trip it; (4) file claims must sit near a file-ish object (filename/file/disk/config/memory) — "I've updated SOUL.md" with no write still gets caught, "I've created a table below" walks free; (5) future intent ("I'll add…") is no longer flagged — promise-then-stop turns were already the Persistence Nudge's job. 30-case regression battery green (all previous true positives still caught).
- 🖥️ **LM Studio support**: LM Studio now works as a first-class local backend alongside Ollama, seamlessly interchangeable. Its models auto-appear in the model selector (🖥️ group) whenever LM Studio's local server is running — switch backends just by picking a model. New `lmstudio_manager.py` mirrors the Ollama subsystem (health + `/v1/models` discovery + live model list); routing goes through the standard OpenAI-compatible path (no API key, streaming on). Opt-in via a `providers.lmstudio` config section (`enabled: true`, default `baseUrl: http://localhost:1234/v1`), so non-users never poll for it.
- 📏 **LM Studio prompt diet + context clamp**: fixed `request (N tokens) exceeds the available context` 400s (a bare "hello" was shipping ~26k tokens!). Root cause was double: `lmstudio` was missing from the `supports_native_tools` provider list, so the gateway fell back to **injecting every tool schema as text** into the system prompt (~20k tokens of "AVAILABLE TOOLS" + few-shot examples); and the local-model tool-overload guard only covered Ollama, so LM Studio got the broad ~60-100 tool set instead of the slim ≤28 (⌐14 coding) local set. LM Studio now runs native tool calling with the same slim toolset and slim-prompt path as Ollama. On top of that, `_get_context_window_for_model` treats LM Studio's discovered **loaded** context as a hard cap — LM Studio loads models with a *fixed* window (no per-request `num_ctx`), so configured values get clamped to it (conservative 8k default when undiscovered), and the manager prefers `loaded_context_length` over the architectural `max_context_length`. A context-overflow rejection now also logs an actionable hint instead of just a cryptic 400. Tip: for the full experience load your LM Studio model with Context Length ≥ 16k (32k comfortable) — below that Galactic trims history hard to fit.
- 🎯 **CONTEXT USED is finally real**: the Status tile / CTX meter estimated context from a chars÷4 guess of chat history alone — ignoring the system prompt, tool schemas, and injected memories, i.e. most of the actual request. Root cause found in the stream parser: OpenAI-style providers send their final usage report in a frame with **empty `choices`**, and the parser's early `continue` skipped it — so real token counts were never captured for any streamed call. Fixed: usage is captured from that final frame, preserved across calls (`_last_usage_final`), preferred by the context meter, and reset by `/clear`. Cost tracking inherits the real numbers too.
- 🧼 **Status tab cleanup**: removed the Software Update section — it hardcoded "v2.1.0" and its Check button was a stub; updates live in Settings, and the dismissible topbar banner (WS `update_available`) actually works again now that a duplicate function stopped shadowing it. Removed the retired **Legacy (Classic)** interface theme (server always serves Modern now, even with a stale cookie) — all Visual Theme skins (Cyberpunk, Matrix, Star Wars, …) untouched.
- 🗂️ **Project Workspaces + searchable conversation history (Antigravity-style)**: Galactic now has a first-class notion of *which codebase you're working on*. **Workspaces**: mention any real folder in chat ("scan `C:\workspace\PumpTrader`") and it auto-registers + becomes the **active workspace** (logged as `📁 Active workspace → PumpTrader`) — injected into the agent's environment ("this codebase" now always resolves correctly), into the Architect's planning baton (a re-spawned planner can never explore the wrong repo again), and persisted across restarts in config. The session bar gains a **📁 workspace chip** with a switcher menu (recent projects, missing-folder detection, add-by-path, forget), `GET/POST /api/workspaces*` endpoints, and the CLI gets **`/workspace`** (`/ws`) — list + switch by name or path, with the active project shown in the CLI status bar. **Conversations**: the session dropdown is now an Antigravity-style **searchable convo picker** — type to filter, rows show message count + relative age + a **📁 workspace tag** (sessions are tagged with the project they belonged to at save time via sidecar metadata), one click to resume, "＋ New conversation" on top. Existing save/switch/delete/export flows unchanged underneath.
- 🏃 **The relay race actually passes the baton now — plans survive between messages**: the Architect would finish a long planning run, hand off… and the moment you sent the next message ("start phase 1"), everything reset: `active_plan` lived in per-request context, so **the plan evaporated between messages**, the next coding message re-planned from scratch, and the freshly-spawned Architect received just your three words — no conversation, no previous plan, no project path — so Kimi explored the *default workspace (Galactic's own repo)* instead of your project. Fixed end to end: **(1)** main-chat plans persist across messages (isolated agents stay isolated); **(2)** continuation turns inject the surviving plan + Architect blueprint into context, logged as `🧬 Continuing existing plan (N steps)`; **(3)** a brand-new coding task retires the old plan and re-plans fresh, follow-ups continue it; **(4)** when the Architect *is* re-spawned it now receives a **baton** — previous plan (with the original task and its project path) + the last 6 conversation messages — so it can never lose the target again; **(5)** "start/begin/execute phase N" phrasing recognized as coding follow-ups; **(6)** `/clear` retires plans fully. Verified with cross-request context tests.
- 🛡️ **Strict tool-pairing sanitizer — the Architect can't die at the finish line anymore**: with native tools live, Kimi K3 explored PumpTrader beautifully for 2.5 minutes (15 file reads across 10 tool turns) and then the whole run 400'd: *"an assistant message with 'tool_calls' must be followed by tool messages responding to each 'tool_call_id'"* — one dropped reply in a 3-tools-at-once turn violated Moonshot's strict OpenAI protocol and poisoned the conversation. Every outgoing OpenAI-compat request now passes `_sanitize_tool_pairing`: missing tool replies get a stub answer, orphan/duplicate/unknown-id replies are dropped, so a single lost result can no longer kill a planning session (lenient providers are unaffected). Forge Sentinel also stops treating these 400s as system faults.
- 📏 **CTX meter no longer shows the Architect's context as the main model's**: while Kimi planned in its isolated session, its 20k+ prompt-token usage frames were written into the shared usage snapshot, so the deck's CTX bubble (and "context used" tile) climbed alarmingly against the *Ollama* window — looking exactly like the two models were sharing one context. They never were: sessions are isolated; only the *gauge* leaked. All usage-capture sites now mirror into the meter's snapshot **only from the main chat**, isolated agents keep their own numbers for cost tracking, and the meter reads the main-chat snapshot exclusively.
- 🔧 **Moonshot/DeepSeek get native function calling — the Architect can finally explore**: a live hybrid run billed real Moonshot credits (~$0.10, confirmed on the balance after a dashboard lag) but produced a plan whose own first line admitted *"the exploration was interrupted before any tool results came back"* — Kimi K3's tool calls never executed (zero tool runs during the whole planner window). Cause: `supports_native_tools` listed `kimi` but not `moonshot` (the provider actually in use), so the Architect received tools as a legacy text blob, its text-JSON tool attempts failed to parse, and it planned blind. Added `moonshot` + `deepseek` to the native-tools providers (Moonshot's `/models` itself advertises `supports_dynamic_tools: true`). Bonus: the giant text tool-block disappears from Architect prompts, cutting thousands of input tokens per planning call.
- 🧬 **Hybrid Architect bulletproofing — persona-free planning + follow-up awareness**: the logs finally proved the Architect (Moonshot/Kimi K3) IS dialed with the user's chosen models (23,411 real input tokens billed on the audit run) — but it answered its 23k-token planning request with *"Right on, let me crack this thing open…, man"* (76 chars of persona small-talk) because the isolated planner inherited the full chat persona **plus** injected identity memories ("Chong runs on Kimi K3"). And follow-ups like "tackle the critical issues first" never engaged hybrid at all (no coding verb/noun matched). Four fixes: **(1) plain-persona planner** — `speak_isolated(plain_persona=True)` swaps the persona for a strict senior-architect identity and skips semantic-memory injection entirely (per-model `system_prompt` overrides still win); **(2) junk-plan rejection** — a 1-step, <300-char, untagged "plan" is recognized as chatter and the next planner attempt runs instead of executing garbage; **(3) mandatory deliverable** — the planning request now ends with an explicit "END with `<plan>…</plan>` or the run failed" demand; **(4) coding follow-up window** — a coding turn arms the next 6 turns, so continuation imperatives ("tackle/handle/address/go ahead/do the rest/start with #2") stay on the coding/hybrid pipeline; casual turns tick the window down. Verb list also gained tackle/handle/address/resolve/solve/apply/work and the code-object list gained issue/bot/algorithm/strategy/latency — "tackle the critical issues first" now triggers hybrid *fresh*, armed or not. 30-case detection suite + persona-swap + junk-rejection tests green.
- ♻️ **Deleted model overrides no longer resurrect**: per-model overrides (and aliases) deleted from the deck's Models tab kept re-appearing after a reload. Root cause: config loads as `config.yaml` (tracked template) ← `config.local.yaml` (overlay) via `deep_merge`, and deep-merge can only *add/override* keys — never *delete* one that exists in the base. Any override sitting in `config.yaml` was therefore un-deletable; the merge re-injected it every load. The save side already treated `model_overrides`/`aliases` as whole-value replacements, but the load side didn't — they disagreed. Fixed: `config_loader.deep_merge` now treats those keys as **overlay-authoritative** (the overlay's value replaces the base's, so deletions stick), and the stale entries were purged from `config.yaml`/`config.template.yaml` (they'd leaked into the tracked template, where user tuning doesn't belong). The template now documents `model_overrides: {}` as the intended empty default.
- 🎨 **CLI now looks like a real app, not a terminal**: adopted the same `prompt_toolkit` chrome that gives Gemini/Claude-style CLIs their feel — a **framed input** (top rule + `│ ❯` bordered prompt line), a **persistent bottom status bar** under the input (live `⬡ model │ ctx % │ 📁 cwd │ ⚡mode-flags │ /help · /model · ^C stop`), **inline history suggestions** (fish-style ghost text you accept with →), a **styled completion dropdown** (the "selectable box" for `/commands` and `@files`), placeholder hint text, and a gradient **GALACTIC ASCII wordmark** on connect. All theme-colored, with an ASCII-only fallback for legacy consoles. (A fully pinned full-screen frame — bordered box that stays put while output scrolls above it — is a larger `Application` rewrite; this delivers the look without destabilizing the streaming output that works today.)
- ⌨️ **CLI professional pass**: startup banner (version · personality · active model · fallback · local backends · uptime); **persistent command history** across sessions (`~/.galactic_cli_history`, arrow-up recalls yesterday); **`/model`** command — `/model` shows active/primary/fallback, `/model kimi` fuzzy-switches across configured cloud models *and* live Ollama/LM Studio discoveries (picker on multiple matches, `provider/model` for exact); the prompt now shows the **live model badge** (`[PumpTrader ⚡kimi-k3] ❯`) kept fresh via telemetry — it updates when fallbacks or hybrid handoffs change the model mid-session; **Ctrl+C mid-turn sends STOP to the core** on exit so an abandoned turn stops burning tokens; a **session summary** (duration · tokens · est. cost) prints on exit; `--version` flag.
- 📐 **True context-window auto-detection for cloud models (Kimi K3 = 1M, not 32k)**: the deck showed `kimi-k3` capped at 32,768 because context resolution had no moonshot rule and fell to a generic default — while Moonshot's own `/v1/models` reports `context_length: 1,048,576`. The gateway now **asks the provider**: a one-shot background fetch of `GET /v1/models` caches every model's real `context_length` (works for Moonshot, OpenRouter, and any OpenAI-compat provider that publishes it), with fresh static fallbacks (`kimi-k3` → 1M, other Kimi → 256k, `moonshot` provider default 256k) for the first call or offline use. Precedence unchanged: per-model override > global `context_window` > live-detected > name rules. The CTX meter, Status tile, and history trimming all follow automatically — no more hand-setting context per model.
- 🧹 **Topbar decluttered**: removed the vestigial `↑0 ↓0 tokens` counter (it rarely updated and duplicated the Status tab's real counters); the clickable CTX/Icebox meter stays. The LM Studio health row now shows the **loaded context** next to the model count (e.g. `2 models · ctx 32k`) — the number LM Studio hard-caps requests at, so trimming behavior is no longer a mystery.
- 🧯 **LM Studio idle auto-unload (VRAM guard)**: every LM Studio request now carries a `ttl` (default 600s, `providers.lmstudio.ttl`, 0 disables) so an idle LM Studio model **unloads itself and frees VRAM** instead of sitting loaded while Ollama loads its own 27B next to it — the double-residency that OOM-crashes the GPU. Older LM Studio builds simply ignore the field. Manual gains a VRAM-safety recipe (24–32k context, K/V cache quantization, watch the load estimate; GPU-strict LM Studio does *not* spill to system RAM the way Ollama does).
- 🖥️ **CLI can now answer approval gates and `ask_user` — no more running to the deck**: the terminal CLI received the Crucible `approval_request` and `ask_user` events over its WebSocket all along but ignored them, so an approval-gated coding run driven from the CLI could only be answered in the web deck (which made the CLI pointless for gated work). Now it renders the colorized diff / question **inline in the terminal** and posts your decision straight back (`/api/approval/respond`, `/api/ask_user/respond`): approve with `y`, reject with `n`, or type a rejection reason as free text; `ask_user` prompts for a free-text answer. Decisions are deduped and race-safe against the deck answering first (whoever answers first wins; the other side shows "already resolved"). New `/autoapprove [on|off]` toggle auto-accepts write gates for hands-off autonomous runs (server-side diff + VCR backup still recorded), with an `[AUTO-APPROVE]` indicator in the prompt so it's never a silent surprise.
- ⏱️ **Approval gate no longer self-destructs on a 10s timeout**: with `require_approval: true`, `write_file`/`edit_file`/`replace_function` block inside the tool waiting on your Approve/Reject click (up to `approval_timeout`, default 300s) — but that human wait was wrapped by the tool's own dispatch timeout, which for write_file is **10s**. So every gated write was killed 10 seconds in, long before anyone could click, surfacing as `write_file raised: Timeout (took longer than 10s)`; the model then misreads it as "file too large" and thrashes on retries. `_get_tool_timeout` now returns `max(base, approval_timeout + 30)` for the approval-gated tools when approval is on, so the human decides, not the clock. (Approval off = timeouts unchanged; config `tool_timeouts` overrides still win.)
- 🧬 **Hybrid coder now actually uses the Architect model (Moonshot/Kimi K3) — the Architect was secretly running as the Builder**: with `architect: moonshot/kimi-k3` + `builder: ollama`, the log said "Spawning isolated planner agent (moonshot/kimi-k3)" but **every LLM call was Ollama and Moonshot charged zero credits** — the Architect (the "brains") was silently doing nothing while the local Builder did all the thinking *and* the work. Root cause: the isolated planner runs the same `_speak_logic`, re-detected "scan this codebase" as a coding task, and called `_apply_hybrid_builder()` — which overwrote its own Moonshot model with the local Ollama Builder. The Architect clobbered itself into the Builder before it ever dialed the cloud. Fixed with a hard guard: `_apply_hybrid_builder` is now a no-op inside any isolated agent (`_session_trace_sid` set), and the planner is spawned with `session_id="planner"` so `hybrid_on` is False inside it. Now the Architect runs on Moonshot (cloud brains → plan/blueprint) and the Builder runs on Ollama (local hands → apply/output), which is the whole point of the split. (Also fixed the earlier key-resolution half of this: `_get_provider_api_key` returned the *live* session key regardless of which provider was asked for, so a leftover Google fallback key would 401 Moonshot — now `ignore_live=True` resolves an override provider's key strictly from config, and `speak_isolated` seeds the isolated agent with the override provider's own key.)
- 🛡️ **Forge Sentinel no longer amplifies errors into a cascade**: on any `[ERROR]` it analyzed the log and then called `gateway.speak()` with its *own* alert text — re-running the full agentic pipeline, which re-detected "coding" intent and launched *another* hybrid planner/builder, whose errors tripped the Sentinel again (a self-amplifying loop that clobbered whatever the user was doing). Now it delivers the alert as a plain chat notification (emit + persist, no agent turn), runs its analysis with `skip_planning=True`, and its skip-list covers LM Studio context-overflow wording (`exceeds the available context`, `Context size has been exceeded`, `predict request returned 400`) and Gemini's transient `thought_signature` error — so it stops reacting to errors it can't fix.
- 🎯 **Coding-intent detection — both misfires and misses fixed**: two symptoms, one detector. (1) *False positive:* "Nice, it worked!! Changed a setting and testing again." launched the whole Senior-Coder + hybrid Architect/Planner/Builder pipeline (cloud planner call included) because detection matched bare everyday verbs ("changed"). (2) *False negative:* "scan this codebase and offer improvements" did **not** trigger hybrid at all — "scan"/"offer" weren't coding verbs, and the planning gate checked for the literal string "scan **the** codebase" so "scan **this** codebase" slipped through. Result: the hybrid Architect (Moonshot/Kimi K3) and Builder (LM Studio) the user had selected were never invoked. Detection is now two-signal: weak verbs — now including review/analysis verbs (scan, review, analyze, audit, optimize, improve, inspect, profile) — only count when the message also names a code-ish object (codebase, bug, file, config, a `.py`-style extension, inline `code`, …); unambiguous verbs (refactor, debug, implement) stand alone; `/code` still forces it. The brittle "scan the codebase" literal is now a determiner-agnostic regex (the/this/my/our/your + code/codebase/repo/project, optional "map **out** this repo" adverb). 39-case regression suite: casual chat ("review my resume", "optimize my morning routine") stays out, real work ("scan this codebase…", "audit the codebase for security issues") gets in.
- 🔀 **Local-backend switcher in the Control Deck**: the topbar pill (formerly "Ollama: N models") now shows **both** local backends ("Ollama: 3 · LM Studio: 2") and clicking it opens a quick-switch menu — every Ollama and LM Studio model one click away, with health dots and the active model highlighted; "Manage all models →" jumps to the Models tab. The Models tab gains an LM Studio health row (with Refresh) and a violet 🖥️ model section beside Ollama's orange one; LM Studio is also selectable in **Settings → Model Configuration** provider dropdowns (Primary/Fallback/Boost/Planner/etc., discovered models included), in the ⚙ Setup wizard (with server-URL field; saving hot-activates the manager, no restart), and reports in **Status → Local Backends** + diagnostics. Fixed the race where the LM Studio model section was wiped by the config model-list load (`ALL_MODELS` rebuild now re-injects cached local discoveries), exempted `lmstudio` from the API-key gate in `/api/switch_model`, and hardened `switchModel` against a missing legacy `cfg-provider` element that silently aborted post-switch UI updates.
- 🌙 **Moonshot / Kimi K3**: Fixed a bug where selecting a `moonshot/*` model errored with *"Request URL is missing an 'http://' or 'https://' protocol"* — the `moonshot` provider had no base URL registered. Added `https://api.moonshot.ai/v1`, refreshed the model list to the live lineup (**Kimi K3**, K2.7 Code, K2.7 Code Highspeed, K2.6 — the retired `kimi-k2.5` is gone), and enabled streaming for Moonshot. Also fixed a latent routing bug: a redundant self-namespaced model id (e.g. `moonshot/kimi-k3` on the moonshot provider) is now stripped to the raw id the API expects — this also hardens other direct providers.
...
- **Loop Reset on User Input**: Internal tool repetition history now clears every time the user sends a message, preventing "stuck" loops after manual state changes.
- **Browser Hallucination Guard**: Added active detection for models claiming to perform browser actions (clicking/typing) without calling tools, with automatic corrective nudges.
- **Fallback Resilience**: Hardened the model/provider transition logic to prevent authentication errors (401) and provider-ID mismatches during auto-fallback.
- **Aggressive Browser Discovery**: Increased browser loop limits for complex navigation tasks.
