...
A powerful, local-first AI automation platform with 188+ built-in tools, an extensible Skills ecosystem, true persistent memory, voice I/O, video generation, Chrome browser extension, social media tools, 14+ AI providers, multi-platform messaging bridges, and a real-time web Control Deck. **v1.6.1**
...
| **Thinking** | Real-time agent trace — watch the ReAct loop think and act step by step; persists across page refreshes; resumable workflow checkpoints |
...
| **Tools** | Browse all 150+ built-in tools with descriptions and parameters |
| **Skills** | Enable/disable core and community skills with one click; shows icon, version, author, tool count |
...
| **Logs** | Real-time log stream with tool call highlighting, smart auto-scroll, and 500-line history |
...
├── gateway_v3.py             # LLM routing + 150-tool ReAct loop
├── web_deck.py               # Web Control Deck (http://127.0.0.1:17789)
├── launcher_desktop.py       # Desktop app launcher (pywebview wrapper)
├── remote_access.py          # JWT auth, rate limiting, CORS middleware
...
├── build_exe.ps1             # PyInstaller Windows EXE builder
...
├── skills/
│   ├── core/                 # Built-in skills (shell, desktop, chrome, social, subagent)
│   └── community/            # AI-authored and user-installed skills (auto-loaded)
└── scripts/
    └── release.py            # Automated release builder
```
...
**`GalacticAI.exe` closes immediately?**
Check `logs/desktop_launcher.log` in your Galactic AI folder — it captures all startup errors. Also ensure `config.yaml` is in the same folder as the exe (or its parent folder).

**`GalacticAI.exe` shows the Setup Wizard every launch?**
The exe can't find your existing `config.yaml`. Make sure there is no `config.yaml` inside the `dist/` folder — delete it if so. The exe looks one level up.

**Can't copy/paste text in the desktop app?**
This was fixed in v1.3.0. Make sure you're running the latest exe from the v1.3.0 release.
...
| Version | Highlights |
|---|---|
| v1.6.1 | Tooling | **Loop Reset & Hallucination Defense** • Multi-turn loop detection resets on new input • Browser Hallucination Guard to catch lying models • Hardened provider-aware fallback resilience |
